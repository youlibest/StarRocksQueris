...
```sql
-- 创建数据库（如果不存在）
CREATE DATABASE IF NOT EXISTS chengken CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `starrocks_information_connections` (
  ...
  `feip` varchar(200) NOT NULL COMMENT '集群连接地址',
  `user` varchar(200) NOT NULL COMMENT '集群登录账号',
  `password` varchar(500) NOT NULL COMMENT '集群登录密码',
  `feport` int NOT NULL DEFAULT '9030' COMMENT '集群登录端口',
  `address` varchar(500) DEFAULT NULL COMMENT 'MANAGER地址',
  `expire` int DEFAULT '30' COMMENT 'LICENSE过期提醒天数',
  `status` int NOT NULL DEFAULT '0' COMMENT 'LICENSE检查开关',
  `fe_log_path` varchar(500) DEFAULT NULL COMMENT 'FE日志目录',
  `be_log_path` varchar(500) DEFAULT NULL COMMENT 'BE日志目录',
  `java_udf_path` varchar(500) DEFAULT NULL COMMENT 'UDF目录',
  `manager_access_key` varchar(500) DEFAULT NULL COMMENT 'access key',
  `manager_secret_key` varchar(500) DEFAULT NULL COMMENT 'secret key',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB COMMENT='StarRocks登录配置';

-- 慢查询配置表
CREATE TABLE `starrocks_information_slowconfig` (
  `slow_query_time` int NOT NULL DEFAULT '600',
  `slow_query_ktime` int NOT NULL DEFAULT '1500',
  `slow_query_concurrencylimit` int NOT NULL DEFAULT '80',
  `slow_query_version` varchar(500) DEFAULT NULL,
  `slow_query_focususer` varchar(2000) DEFAULT NULL,
  `slow_query_proxy_feishu` varchar(300) DEFAULT NULL,
  `slow_query_grafana` varchar(200) DEFAULT NULL,
  `slow_query_lark_app` varchar(100) DEFAULT NULL,
  `slow_query_lark_appid` varchar(200) DEFAULT NULL,
  `slow_query_lark_appsecret` varchar(100) DEFAULT NULL,
  `slow_query_email_host` varchar(200) DEFAULT NULL,
  `slow_query_email_from` varchar(200) DEFAULT NULL,
  `slow_query_email_to` varchar(200) DEFAULT NULL,
  `slow_query_email_cc` varchar(100) DEFAULT NULL,
  `slow_query_email_bc` varchar(200) DEFAULT NULL,
  `slow_query_email_suffix` varchar(100) DEFAULT NULL,
  `slow_query_email_reference_material` varchar(2000) DEFAULT NULL,
  `slow_query_frontend_avgs` varchar(2000) DEFAULT 'parallel_fragment_exec_instance_num=15,query_mem_limit=274877906944',
  `slow_query_frontend_fullscan_num` int DEFAULT '200000000',
  `slow_query_frontend_insert_catalog_scanrow` int DEFAULT '100000000',
  `slow_query_frontend_memoryusage` int DEFAULT '200',
  `slow_query_frontend_scanrows` bigint DEFAULT '10000000000',
  `slow_query_frontend_scanbytes` int DEFAULT '5',
  `slow_query_data_registration_username` varchar(100) DEFAULT NULL,
  `slow_query_data_registration_password` varchar(500) DEFAULT NULL,
  `slow_query_data_registration_table` varchar(500) DEFAULT NULL,
  `slow_query_data_registration_host` varchar(200) DEFAULT NULL,
  `slow_query_data_registration_port` int DEFAULT '8030',
  `slow_query_resource_group_cpu_core_limit` int DEFAULT '10',
  `slow_query_resource_group_mem_limit` int DEFAULT '50',
  `slow_query_resource_group_concurrency_limit` int DEFAULT '3',
  `slow_query_metaapp` varchar(100) DEFAULT NULL,
  `slow_query_auditload` varchar(500) DEFAULT NULL,
  `slow_query_wecom_webhook_key` varchar(500) DEFAULT NULL COMMENT '企业微信Webhook Key',
  `slow_query_wecom_msg_type` varchar(50) DEFAULT 'markdown',
  `slow_query_wecom_mention_all` int DEFAULT '0',
  `slow_query_wecom_status` int DEFAULT '0',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB;
...
CREATE TABLE `starrocks_information_larkrobot` (
  ...
  `key` varchar(100) NOT NULL COMMENT '集群通知标记',
  `robot` varchar(500) DEFAULT NULL COMMENT '飞书机器人KEY',
  `status` int NOT NULL DEFAULT '0' COMMENT '开关',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB COMMENT='慢查询告警推送机器人';
...
CREATE TABLE `starrocks_information_wecomrobot` (
  ...
  `mentioned_mobile_list` varchar(500) DEFAULT NULL COMMENT '手机号列表',
  `status` int NOT NULL DEFAULT '0' COMMENT '开关',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
...
```sql
-- 插入慢查询配置（使用配置文件中的默认值）
INSERT INTO `starrocks_information_slowconfig` (
  `slow_query_time`, `slow_query_ktime`, `slow_query_concurrencylimit`
) VALUES (600, 1500, 80);

-- 插入集群连接配置
INSERT INTO `starrocks_information_connections` (
  `app`, `feip`, `user`, `password`, `feport`
) VALUES (
  'prod-cluster', '192.168.1.100', 'root', 'your-password', 9030
);

-- 插入飞书机器人配置（可选）
INSERT INTO `starrocks_information_larkrobot` (
  `type`, `key`, `robot`, `status`
) VALUES (
  'global', 'default', 'your-feishu-robot-key', 1
);

-- 插入企业微信机器人配置（可选）
INSERT INTO `starrocks_information_wecomrobot` (
  `type`, `key`, `webhook_key`, `msg_type`, `status`
) VALUES (
  'global', 'default', 'your-wecom-webhook-key', 'markdown', 1
);
```
